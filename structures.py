...
class PostAttribute:
	...
	def __init__(self, file: Path):
		...
		file_attributes = list(map(lambda s: s.strip(), file_attributes.strip()[4:-3].strip().split(';')))
		self.title: str = file_attributes[0] if len(file_attributes) > 0 else ''
		self.tags: list[str] = list(map(lambda s: s.strip(), file_attributes[1].split(','))) if len(file_attributes) > 1 else []
...
class PostIndex:
	...
	def date(self, file: Path) -> tuple[int,struct_time,str]:
		...
		idx = self.find(file)
		creation_date = (self.index[idx].creation_date if idx != -1 else strftime('%Y-%m-%d\\%H:%M:%S\\UTC%z', date))
