...
class PostAttribute:
    ...
    def __init__(self, file: Path):
        ...
        self.tags: Optional[str] = list(map(lambda s: s.strip(), file_attributes[1].split(','))) if len(file_attributes) > 1 else None
...
class PostIndex:
    ...
    def find(self, file: Path) -> int:
        ...
        for (i,entry) in enumerate(self.index):
            if entry[0] == file_str:
                return i
        return -1
    def append(self, entry: tuple[str,str,str]):
    ...
    def __getitem__(self, index: int) -> tuple[str,struct_time,str]:
        return self.index[index]
    def date(self, file: Path) -> tuple[int,str,str]:
...
class PostTree:
    ...
    def __init__(self, index: PostIndex):
        post_list = PostTree.__create_post_list(index.index)
        ret = []
    ...
    @staticmethod
    def __create_post_list(index: list[tuple[str,str,str]]) -> list[tuple[str,str,str,str]]:
        ret = []
        for (filename_, _, title) in index:
...
def create_post_html_name(year: str, month: str, post_tree: list[tuple[str,str]]) -> str:
    ...
    for (name, title) in post_tree:
        ret.append(f'<a href="/posts/{year}/{month}/{name}">{title}</a>')
    return '<br/>\n'.join(ret)
...
def fill_template(template: str, creation_date: str, date: str, content: str, title: Optional[str], sidebar: str) -> str:
...
def main():
    ...
    result = fill_template(template, creation_date, date, content, post_attributes.title, post_sidebar)

    if index_index == -1:
        post_index.append((str(args.file), creation_date, post_attributes.title))
        post_index.save()
